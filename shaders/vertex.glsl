...
float getBlob(vec3 position){
    vec3 wrappedPosition = position * uPositionFrequency;
    wrappedPosition += cnoise(position * uSmallWavePositionFrequency + u_time * uTimeFrequency) * uSmallWavePositionStrength;
    float noise = cnoise(wrappedPosition * uSmallWavePositionFrequency + u_time * uSmallWaveTimeFrequency);
    // float bubble = smoothstep(-2.0, 1.8, noise);
    return noise * uPositionStrength;
}
